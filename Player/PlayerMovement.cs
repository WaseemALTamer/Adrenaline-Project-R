...
public class Playermovement : NetworkBehaviour
{
    ...
    Vector3 move_foraward(float axis_local,float angle, bool GetLocalAxis){
        ...
        if (GetLocalAxis == true){
            yRotation = rb.transform.eulerAngles.y;
        }
    ...
    void Update()
    {
        ...
        if (Colliding_On_y)
        {
            ...
            {
                JumbServerRpc(new Vector3(0,jumpForce,0));
            }
        }

        rotationX += Input.GetAxis("Mouse X") * rotationSpeed;
        rb.transform.localRotation = Quaternion.Euler(0, rotationX, 0f);
        ...
        MovementServerRpc(movement * Time.deltaTime);
    }


    [ServerRpc]
    private void MovementServerRpc(Vector3 movement){
    ...
    [ServerRpc]
    private void JumbServerRpc(Vector3 Jumb){
        rb.AddForce(Jumb,ForceMode.Impulse);
    }
}